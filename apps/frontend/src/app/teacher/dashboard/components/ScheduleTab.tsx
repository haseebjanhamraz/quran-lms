    ...
    <section className="grid grid-cols-1 xl:grid-cols-3 gap-6">
      ...
      <div className="xl:col-span-2 space-y-5">
        ...
        {sessionsLoading ? (
          ...
          <div className="rounded-2xl border border-border bg-card/60 py-16 text-center backdrop-blur-sm">
            <p className="font-semibold text-muted-foreground">No sessions scheduled yet.</p>
          </div>
        ) : (
          <div className="flex flex-col gap-3">
            {sessions.map((session, i) => (
              <div key={session.id} className="relative">
                ...
                <div className="flex flex-col gap-4 rounded-2xl border border-border bg-card/80 p-5 backdrop-blur-sm transition-all duration-200 hover:border-brand/40 hover:shadow-lg sm:flex-row sm:items-center sm:justify-between shadow-sm">
                  <div className="flex items-start gap-4 sm:items-center">
                    ...
                    <div>
                      <div className="flex flex-wrap items-center gap-2">
                        <p className="font-semibold text-foreground">{session.course.title}</p>
                        <span className={`rounded-full border px-2 py-0.5 text-xs font-medium ${TYPE_COLORS[session.course.type] || 'text-muted-foreground bg-muted/60'}`}>
                          {typeLabel(session.course.type)}
                        </span>
                        {session.status === 'LIVE' && (
                          <span className="flex items-center gap-1 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-2 py-0.5 text-xs font-medium text-emerald-600 dark:text-emerald-400">
                  ...
                  <div className="flex flex-shrink-0 items-center gap-2 self-end sm:self-auto">
                    {session.status === 'SCHEDULED' && (
                      <>
                        ...
                        <button
                          ...
                          className="rounded-xl border border-border px-3 py-1.5 text-xs font-medium text-foreground/80 hover:border-brand/40 hover:text-foreground"
                        >
                          Edit
                        </button>
                        <button
                    ...
                    {session.status === 'COMPLETED' && (
                      <button
                        ...
                        className="flex items-center gap-1.5 rounded-xl border border-teal-500/50 px-4 py-1.5 text-xs font-semibold text-teal-600 dark:text-teal-400 transition-all duration-200 hover:bg-teal-500/10"
                      >
                        <Award size={14} />
                        View Feedback
                      </button>
                    ...
                    {session.status === 'CANCELLED' && (
                      <span className="rounded-xl bg-muted px-4 py-1.5 text-xs font-medium text-muted-foreground border border-border">
                        Cancelled
                      </span>
                    )}
                  </div>
      ...
      <div className="space-y-5">
        <div className="flex justify-between items-center">
          ...
          <Link href="/teacher/feedback" className="text-xs text-brand hover:underline font-semibold">
            See all &rarr;
          </Link>
        </div>

        <div className="rounded-2xl border border-border bg-card/80 p-5 backdrop-blur-sm space-y-4 shadow-sm">
          {reviewsLoading ? (
            <div className="py-10 flex justify-center">
              <Loader2 size={24} className="animate-spin text-brand" />
            </div>
          ) : recentReviews.length === 0 ? (
            <p className="text-sm text-muted-foreground text-center py-6">No evaluation scorecards yet.</p>
          ) : (
            <div className="space-y-4">
              {recentReviews.map((rev) => (
                <div
                  ...
                  className="cursor-pointer border-b border-border/60 pb-3 last:border-0 last:pb-0 hover:bg-muted/40 p-1.5 rounded transition-all"
                >
                  <div className="flex justify-between items-start gap-2">
                    <p className="font-semibold text-xs text-foreground truncate">{rev.session.course.title}</p>
                    <span className="text-xs font-mono font-bold text-amber-500">{rev.overallScore.toFixed(1)}/5.0</span>
                  </div>
