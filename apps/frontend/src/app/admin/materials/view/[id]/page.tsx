...
function PdfPageItem({
  ...
  // Page 1 should start rendering immediately
  ...
  // 1. Intersection Observer for lazy rendering & visibility tracking
  useEffect(() => {
  ...
  // 2. Render canvas via PDF.js whenever in view or parameters change
  useEffect(() => {
  ...
  return (
    ...
    >
      {/* Page Canvas Container with Shadow and Subtle Border */}
      <div
      ...
      >
        ...
        {/* Diagonal Repeating Security Watermark */}
        <div
        ...
        {/* Transparent Security Shield Overlay (blocks right-click and canvas drag) */}
        <div
...
export default function ProtectedMaterialViewerPage() {
  ...
  // 2. Load PDF.js library locally from /pdfjs/ and initialize document
  useEffect(() => {
    ...
    const loadPdfJs = async () => {
      try {
        ...
        if (pdfjs) {
          ...
          // Read unscaled base dimensions from page 1
          ...
          // Calculate initial Fit to Width (Full Width View)
  ...
  // 3. Fit to Width handler (Full width view)
  ...
  // 4. Fit to Screen handler (Fits entire page height & width)
  ...
  // 6. Ctrl + Mouse Wheel for smooth zooming inside viewer
  useEffect(() => {
  ...
  // 7. Strict Security & Anti-Download Protection Listeners
  useEffect(() => {
    ...
    // B. Intercept Keyboard shortcuts (Print, Save, Copy, DevTools)
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Block Copy & Select All (Ctrl+C, Ctrl+A)
      ...
      // Block Inspect / DevTools shortcuts
      if (
        e.key === 'F12' ||
  ...
  return (
    ...
    >
      ...
      <header className="h-14 shrink-0 bg-zinc-900/90 border-b border-zinc-800/80 px-3 sm:px-4 flex items-center justify-between z-30 backdrop-blur-md">
        {/* Left Section: Back, Title & Protected Badge */}
        ...
        {/* Center Section: Page Navigation (if multi-page) */}
        {numPages > 1 && (
        ...
        {/* Right Section: Always visible Zoom, Fit to Screen, Fit Width, Rotate, Fullscreen */}
        <div className="flex items-center gap-1.5" onClick={(e) => e.stopPropagation()}>
          {/* Fit to Width Button (Full Width) */}
          <button
          ...
          {/* Fit to Screen Button (Fits full page on screen) */}
          <button
      ...
      {/* Document Viewport with Full Width & Continuous Natural Scrolling */}
      <main
      ...
      >
        ...
        {/* Primary View Mode: High-DPI Canvas Rendering via PDF.js */}
        ...
        {/* Fallback View Mode: Full-Screen Protected Embed with Scaling */}
