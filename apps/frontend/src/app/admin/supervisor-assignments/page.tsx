...
export default function SupervisorAssignmentManagement() {
  ...
  const fetchData = async () => {
    try {
      // Fetch assignments
      const assignRes = await fetch(`${API_URL}/supervisor-assignments`, {
        method: 'GET',
  ...
  const handleRemoveAssignment = async (id: string) => {
    if (!confirm('Are you sure you want to remove this supervisor assignment?')) {
      return;
    }

    try {
      const res = await fetch(`${API_URL}/supervisor-assignments/${id}`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
      });
      ...
      console.error('Error removing supervisor assignment:', err);
    }
  };
                  ...
                  return (
                    <tr key={a.id} className="hover:bg-card/20 transition-colors">
                      <td className="py-4 px-6">
                        <p className="font-semibold text-foreground">{supervisorUser?.name || 'N/A'}</p>
          ...
          <div className="glass-panel w-full max-w-md rounded-2xl p-6 shadow-2xl relative">
            ...
            <form onSubmit={handleAssign} className="space-y-4">
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Select Supervisor</label>
                {supervisors.length === 0 ? (
                  <p className="text-xs text-destructive">No supervisor accounts registered! Please create a Supervisor account first.</p>
                ) : (
                  <select
                    value={supervisorId}
